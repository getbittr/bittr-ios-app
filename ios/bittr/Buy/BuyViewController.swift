...
class BuyViewController: UIViewController, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    @IBAction func copyItem(_ sender: UIButton) {
        ...
        self.showAlert(presentingController: self, title: Language.getWord(withID: "copied"), message: value, buttons: [.dismiss(Language.getWord(withID: "okay"))])
    }
    ...
    func parseNewData(receivedDictionary:NSDictionary) {
        ...
        if let receivedEntity = receivedDictionary["data"] as? NSDictionary {
            ...
            if someDetailsHaveChanged {
                ...
                self.showAlert(presentingController: self, title: Language.getWord(withID: "buyvcupdatedetails"), message: Language.getWord(withID: "buyvcupdatedetails2"), buttons: [.dismiss(Language.getWord(withID: "okay"))])
        ...
        if newMode == "lightning" {
            self.hasAcceptedPushNotifications { hasAccepted in
                if hasAccepted {
                    ...
                    self.showAlert(presentingController: self, title: Language.getWord(withID: "receivenotifications"), message: Language.getWord(withID: "lightningneedsnotifications"), buttons: [.dismiss(Language.getWord(withID: "okay"))])
                }
            }
        } else {
            self.proceedWithApiCall(for: entity, newMode: newMode)
    ...
    func proceedWithApiCall(for entity: IbanEntity, newMode: String, isRetry: Bool = false) {
        ...
        guard BitcoinManager.shared.ldkNode != nil, let pubkey = BitcoinManager.shared.nodeId() else {
            ...
            self.showAlert(presentingController: self, title: Language.getWord(withID: "lightningnotready"), message: Language.getWord(withID: "syncingwallet2"), buttons: [.dismiss(Language.getWord(withID: "okay"))])
            return
        }
    ...
    private func handlePaymentModeError(_ message: String, for entity: IbanEntity) {
        ...
        self.reloadCard(for: entity)
        self.showAlert(presentingController: self, title: Language.getWord(withID: "paymentmodeupdateerror"), message: message, buttons: [.dismiss(Language.getWord(withID: "okay"))])
    }
    ...
    @IBAction func paymentModeQuestionTapped(_ sender: UIButton) {
        ...
        self.showAlert(presentingController: self, title: Language.getWord(withID: "buyvclightning"), message: Language.getWord(withID: "buyvclightningexplanation"), buttons: [.dismiss(Language.getWord(withID: "okay"))])
    }
