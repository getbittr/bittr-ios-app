...
- **Purpose**: Lightning ↔ onchain swap entry. Embeds SwapStatusVC after submit. Also opened from Send to auto-run a suggested swap that pays the recipient when the funds are on the other side: onchain→lightning for a lightning invoice with no channel (CoreToSwap), or lightning→onchain for an onchain address with too little onchain balance (SendToSwap).
- **States**: initial / amount entered / direction picker (alert) / fees alert / syncing alert / auto-run from a Send swap suggestion.
- **Flow**: `shared/flows/features/swap.yaml`, `features/send_swap_suggestion_lightning.yaml`, `features/send_swap_suggestion_onchain.yaml`
...
- **Purpose**: yellow-card explanation page — the transient "incoming €X" page during incoming flows, the fee/help explanations launched from elsewhere (e.g. Transaction fees, Settings lightning question), and the header/body of a push notification (the `.information`, `.htlcExpired` and `.unknown` types all render here).
...
- **Screenshots**: `settings/14_lightning_question.png`, `notification_information/02_question.png`, `notification_information/04_htlc_expired.png`, `notification_information/06_unknown.png` (incoming variant not captured directly — terminal state is TransactionVC).
