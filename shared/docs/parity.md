...
- **Swap Live Activity** — `BittrWidget/SwapLiveActivity.swift` + `SwapActivityAttributes.swift`, rendered in the Dynamic Island / Lock Screen and driven by remote pushes. Maestro can't drive either iOS surface, so it's absent from the screenshot catalog for the same reason the price widget is. Five `SwapPhase` states, each with its own copy/icon/tint. **In scope for the port but not for the v1 Maestro gate**, same precedent as the price widget; specified from source in [`swap-live-activity-spec.md`](swap-live-activity-spec.md). The Android counterpart is an **Android 16 Live Update** (`NotificationCompat.ProgressStyle` + promoted ongoing, already available in the pinned `androidx.core` 1.19.0) — not a Glance widget, and **not** a foreground service. Scheduled in Phase 4 after items 7 (swaps) and 9 (notifications). Unlike the iOS surface, most of it is assertable in a JVM unit test, and whether Maestro can assert the Android notification shade is one untested experiment (spec §7). Copy, icons and the chip text are open designer items (spec §6). Decision: BIT-75.
