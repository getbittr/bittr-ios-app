...
| Seed gate rejects wrong words | done | not started | `onboarding/seed_gate_rejects_wrong_words.yaml` | **Parity-critical (BIT-19).** Recovery is mnemonic-only (BIT-8), so Signup4 is the entire recovery guarantee: past it without the real 12 words and the wallet is unrecoverable. Asserts all three rejection branches of `Signup4ViewController.nextButtonTapped` — valid-but-wrong BIP39 words (`incorrectPhraseAlert`), a non-BIP39 string (`invalidWordsAlert`), an empty field (`missingWordsAlert`) — and that each leaves the flow **still on Signup4**, then that the right words do advance. Decoys are derived from the captured mnemonic so one can never coincidentally be the requested word. Android must pass this unchanged; an implementation that accepts any input, or auto-advances on blur, passes every other flow in the suite. |
