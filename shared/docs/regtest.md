...
- `POST /e2e/mine-blocks` — mines N regtest blocks. Body: `{ blocks, address? }`. When `address` is set, coinbase rewards are paid to that address (useful for funding an onchain balance directly); mine at least **101** blocks in that case — coinbase outputs mature after 100 confirmations. See `shared/flows/scripts/mine_blocks.js` (`output.blocksToMine`, optional `output.mineBlocksAddress`).
