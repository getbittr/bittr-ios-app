...
def verify(repo, records, shots_dir):
    ...
    all_declared = {r["screenshot"] for r in records}
    on_disk = set()
    ...
    # Only shots the flows aim AT this directory can be missing FROM it. A flow
    # may deliberately capture elsewhere — shared/docs/device-checks/ holds the
    # per-device frames (BIT-82), which are kept out of the canonical set on
    # purpose: a differently-sized frame under shared/docs/screenshots/ is the
    # BIT-3 defect. Counting those as missing makes the floor unreachable and
    # sends whoever is at the Mac chasing a shot that must never land here.
    # They are reported as out_of_scope rather than dropped silently.
    prefix = os.path.relpath(root, repo).replace("\\", "/").rstrip("/") + "/"
    if prefix.startswith("../"):
        # --verify pointed outside the repo, so no declared path can match the
        # prefix. Scoping there would silently zero out `missing` and report
        # clean. Compare against everything instead and let the mismatch show.
        declared, out_of_scope = set(all_declared), []
    else:
        declared = {d for d in all_declared if d.startswith(prefix)}
        out_of_scope = sorted(all_declared - declared)
...
def main():
    ...
    if v:
        ...
        if v["out_of_scope"]:
            print(f"        out_of_scope={len(v['out_of_scope'])} "
                  f"(declared outside {os.path.relpath(args.verify)}, not counted): "
                  + ", ".join(v["out_of_scope"]))
    return 0
