...
class K1AdminResetTest {
    ...
    fun forceResetCredential() {
        ...
        // Null password == remove the lock screen, by policy, without knowing the old one.
        //
        // No flags. RESET_PASSWORD_REQUIRE_ENTRY says "do not let another admin change the
        // password until the user has entered it", which has nothing to constrain when the
        // password is being cleared to nothing — and this module's minSdk is 26, so the
        // version guard that used to wrap it could never take its else branch anyway.
        val cleared = dpm.resetPasswordWithToken(admin, null, token, 0)
