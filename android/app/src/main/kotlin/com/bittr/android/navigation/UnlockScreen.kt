...
fun UnlockScreen(
    ...
    if (wrongPin) {
        AlertDialog(
            onDismissRequest = { wrongPin = false },
            title = { Text(UnlockStrings.INCORRECT_PIN) },
            text = { Text(UnlockStrings.INCORRECT_PIN_2) },
            confirmButton = {
                TextButton(
                    onClick = { wrongPin = false },
                    modifier = Modifier.testTag(TestID.Alert.buttonAt(0)),
                ) {
                    Text(UnlockStrings.OKAY)
                }
            },
        )
    }
