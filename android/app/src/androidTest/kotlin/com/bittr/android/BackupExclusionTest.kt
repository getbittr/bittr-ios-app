...
class BackupExclusionTest {
    ...
    /** Distinct per file and per run, so a restored file cannot be mistaken for a leftover. */
    private fun stamp(label: String): String =
        "BIT-101 $label ${ByteArray(8).also(random::nextBytes).joinToString("") { "%02x".format(it) }}"
