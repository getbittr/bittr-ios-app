...

{
  ...
  echo "saying the set was empty."
  echo
  echo "That warning names two causes and asserts neither, because one read of"
  echo "one tree, after both paths have run, cannot attribute the emptiness to a"
  echo "path. On the cloud path \`allowBackup=\"false\"\` makes the package"
  echo "ineligible and an empty set is expected. On the device-transfer path"
  echo "\`allowBackup\` does not apply at all, so ineligibility cannot be the"
  echo "cause; an empty set there means the backup did not complete, which is"
  echo "what runs 107 and 133 hit. The suite plants the canary but no longer"
  echo "asserts it — that was \`canaryReturned\`, read back out of the in-process"
  echo "restore BIT-108 removed — so nothing in the test results answers this."
  echo "See"
