...

{
  ...
  echo "decoy at each path the \`<exclude domain=\"file\">\` entries name. It does"
  echo "not restore. The verdict is host-side: when \`adb root\` succeeds,"
  echo "\`check-backup-set.sh\` greps the backup transport's own on-disk tree."
  echo "Grep the log for \`Backup set inspection\` to see which outcome this run"
  echo "got, and for \`Device-transfer backup\` to see whether there was a set to"
  echo "read."
  echo
