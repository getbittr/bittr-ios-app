            ...
            let task = URLSession.shared.dataTask(with: request) { data, response, dataError in
                ...
                if let error = dataError {
                    ...
                    DispatchQueue.main.async {
                        self.showAlert(title: Language.getWord(withID: "lnurl"), message: Language.getWord(withID: "lnurlfail3"), buttons: [Language.getWord(withID: "okay")])
                    }
                }
                        ...
                        if let actualDataDict = dataDictionary {
                            if let receivedTag = actualDataDict["tag"] as? String {
                                ...
                                if receivedTag == "payRequest" {
                                    ...
                                    DispatchQueue.main.async {
                                        self.showAlert(title: Language.getWord(withID: "lnurl"), message: Language.getWord(withID: "lnurlfail4"), buttons: [Language.getWord(withID: "okay")])
                                    }
                                }
                            }
                        }
                ...
                do {
                    ...
                    if let actualDataDict = dataDictionary {
                        if let receivedInvoice = actualDataDict["pr"] as? String {
                            ...
                            DispatchQueue.main.async {
                                self.showAlert(title: Language.getWord(withID: "payrequest"), message: "\(Language.getWord(withID: "lnurlfail2")) \(receivedDetail)", buttons: [Language.getWord(withID: "okay")])
                            }
                        }
                    }
                } catch {
                                    ...
                                    if let receivedStatus = actualDataDict["status"] as? String {
                                        ...
                                        if receivedStatus == "OK" {
                                            ...
                                            if let receivedReason = actualDataDict["reason"] as? String {
                                                DispatchQueue.main.async {
                                                    self.showAlert(title: Language.getWord(withID: "withdrawrequest"), message: "\(Language.getWord(withID: "lnurlfail1")) \(receivedReason)", buttons: [Language.getWord(withID: "okay")])
                                                }
                                            }
                                        }
                                    }
